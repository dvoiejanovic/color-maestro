...
function findWidestColorRange(rgbValues: TPixelColor[]) {
...
export function groupRgbValues(imageData: Uint8ClampedArray): TPixelColor[] {
  ...
  for (let i = 0; i <= imageData.length - 4; i += 4) {
    ...
    if (
      isValidNumber(imageData[i]) &&
      isValidNumber(imageData[i + 1]) &&
      isValidNumber(imageData[i + 2])
    ) {
      rgbValues.push(rgb)
    }
  }
...
export function rgbToHex(pixelColor: TPixelColor) {
